...
export function CardPaymentForm({ onContinue, isSubmitting, helperText }: CardPaymentFormProps) {
  return (
    <div className="mt-4 rounded-xl border border-[#DBE9F9] bg-white p-6">
      ...
      <p className="mb-4 text-sm text-[#1F5CAB]/90">
        Deine Bestellung wird an den sicheren Stripe-/WooCommerce-Zahlungsdialog weitergeleitet.
      </p>
      {helperText ? (
      ...
      >
        {isSubmitting ? 'Weiterleitung…' : 'Mit Karte fortfahren'}
      </button>
    </div>
  )
}
