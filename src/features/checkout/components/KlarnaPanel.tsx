...
interface KlarnaPanelProps {
  onContinue: () => void
  isSubmitting: boolean
}

export function KlarnaPanel({ onContinue, isSubmitting }: KlarnaPanelProps) {
  return (
    <div className="mt-4 rounded-xl border border-[#DBE9F9] bg-white p-6">
      <h3 className="mb-3 text-base font-semibold text-[#1F5CAB]">Klarna</h3>
      <p className="mb-4 text-sm text-[#1F5CAB]/90">
        Klarna prüft deine Angaben und leitet dich zur Zahlung weiter.
      </p>
      <button
        type="button"
        onClick={onContinue}
      ...
      >
        {isSubmitting ? 'Weiterleitung…' : 'Mit Klarna fortfahren'}
      </button>
    </div>
  )
}
